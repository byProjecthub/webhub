...
<div class="dash">
    <div class="card-header">
        <h2 class="display-7 text-center">Payment Status Report</h2>
    </div>
    ...
    if (isset($_POST['generate_report'])) {
        ...
        // Fetch data from the 'booking' table based on the selected payment status, activity type, and time frame
        // Modify the query to match your database structure and filter criteria
        $query = "SELECT booking_id, Activity_Name, Price, Check_in_Date, Check_out_Date, Number_Of_People, catering_included, status, user_id, booking_date 
                  FROM booking 
                  WHERE (status = ? OR ? IS NULL) 
                  AND (Activity_Name = ? OR ? IS NULL) 
                  AND (booking_date >= ? OR ? IS NULL) 
                  AND (booking_date <= ? OR ? IS NULL)";
        $stmt = $con->prepare($query);
        $stmt->bind_param("ssssssss", $payment_status, $payment_status, $activity_type, $activity_type, $start_date, $start_date, $end_date, $end_date);
        ...
        // Fetch data from the 'booking' table and populate the report data
        ...
        // Generate the payment status report in a table format
        $html = '<table>';
        $html .= '<tr><th>Booking ID</th><th>Activity Name</th><th>Price</th><th>Check-in Date</th><th>Check-out Date</th><th>Number of People</th><th>Catering Included</th><th>Status</th><th>User ID</th><th>Booking Date</th></tr>';
        ...
        foreach ($reportData as $row) {
            ...
            $html .= '<td>' . $row['Check_in_Date'] . '</td>';
            $html .= '<td>' . $row['Check_out_Date'] . '</td>';
