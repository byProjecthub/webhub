...
<body>
  ...
  <div class="dash">
    <div class="card-header">
        <h2 class="display-7 text-center">Admin Report</h2>
    </div>
    <form action="" method="post" id="report-form">
        ...
        <option value="" disabled selected>Please Select</option>
            <option value="activities">Activity List Report</option>
            <option value="Act_statistics">Statistics Report</option>
        </select>
...
if (isset($_POST['generate_report'])) {
    ...
    if ($activity === 'all') {
        // Query to get the total of all activities
        $query = "SELECT SUM(act_price) AS total_price_all_activities FROM activities";
    } else {
        // Query for a specific activity
        $query = "SELECT act_id, act_name, act_description, act_price, act1, act2, act3, act4 FROM activities WHERE act_name = ?";
    }

    $stmt = $con->prepare($query);

    if ($activity !== 'all') {
        // If it's not "Select All", bind the parameter
    ...
    if ($activity === 'all') {
        $html .= '<tr><th>Total Price of All Activities</th></tr>';
        if (isset($reportData[0]['total_price_all_activities'])) {
            $html .= '<tr>';
            $html .= '<td>' . number_format($reportData[0]['total_price_all_activities'], 2) . '</td>';
            $html .= '</tr>';
        } else {
            $html .= '<tr><td>No data available</td></tr>';
        }
    } else {
        // Other cases (specific activity)
        $html .= '<tr><th scope="col">Activity ID</th><th scope="col">Name</th><th scope="col">Description</th><th scope="col">Price</th><th scope="col">Act1</th><th scope="col">Act2</th><th scope="col">Act3</th><th scope="col">Act4</th></tr>';
        foreach ($reportData as $row) {
            $html .= '<tr>';
            $html .= '<td>' . $row['act_id'] . '</td>';
            $html .= '<td>' . $row['act_name'] . '</td>';
            $html .= '<td>' . $row['act_description'] . '</td>';
            $html .= '<td>' . $row['act_price'] . '</td>';
            $html .= '<td>' . $row['act1'] . '</td>';
            $html .= '<td>' . $row['act2'] . '</td>';
            $html .= '<td>' . $row['act3'] . '</td>';
            $html .= '<td>' . $row['act4'] . '</td>';
